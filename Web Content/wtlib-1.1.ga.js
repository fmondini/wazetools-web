...
// - 2025/05 v.1.1.GA - Adapted for UREQ (https://ureq.waze.tools)
//
////////////////////////////////////////////////////////////////////////////////////////////////////
...
class WT_ConsoleClass {
	...
	constructor(logPrefix, isDebugMode) {

		this.DEBUG_FLAG = isDebugMode;
		this.LIB_AUTHOR = 'fmondini';
		this.LIB_VERSION = '1.1.GA';
		this.LIB_RELEASE = '2025-05-20';
		this.SCRIPT_TAG = logPrefix.toUpperCase();

		const PRINT_PREFIX = '[' + this.SCRIPT_TAG + ']';	// LOG Prefix for the console
		const DEBUG_PREFIX = PRINT_PREFIX + ' 👽';			// LOG Prefix for the debug console
...
class WT_FeatureClass {
		...
		try {
			...
			if (isValid) {
				...
				selectedFeatures.ids.forEach(id => {
					...
					const streetName = (selectedFeatures.objectType == 'segment'
						? this.GetSegmentStreetName(id)
						: (selectedFeatures.objectType == 'venue'
							? this.GetVenueStreetName(id)
							: 'UNKNOWN objectType: ' + selectedFeatures.objectType
						)
					);

					const polyLine = (selectedFeatures.objectType == 'segment'
						? this.GetSegmentPolyline(id)
						: (selectedFeatures.objectType == 'venue'
							? null
							: 'UNKNOWN objectType: ' + selectedFeatures.objectType
						)
					...
					const countryCode = (selectedFeatures.objectType == 'segment'
						? this.GetSegmentCountryAbbr(id)
						: (selectedFeatures.objectType == 'venue'
							? this.GetVenueCountryAbbr(id)
							: 'UNKNOWN objectType: ' + selectedFeatures.objectType
						)
					);

					const lockRank = (selectedFeatures.objectType == 'segment'
						? this.GetSegmentLock(id)
						: (selectedFeatures.objectType == 'venue'
							? this.GetVenueLock(id)
							: 'UNKNOWN objectType: ' + selectedFeatures.objectType
						)
					);
					...
					dataArray.push({
						...
						'country'  : countryCode,
						'polyline' : polyLine,
						'type'     : selectedFeatures.objectType
					});
				});
	 ...
	 * Get segment lock
	 */
	GetSegmentLock(segmentId) {

		let lockRank = null;

		try {

			lockRank = (WT_WME_SDK.DataModel.Segments.getById(
				{ segmentId: segmentId }
			).lockRank) + 1;

		} catch(err) {
			lockRank = err.message;
		}
	 ...
	 * Get venue lock
	 */
	GetVenueLock(venueId) {

		let lockRank = null;

		try {

			lockRank = (WT_WME_SDK.DataModel.Venues.getById(
				{ venueId: venueId }
			).lockRank) + 1;
	 ...
	 */
	GetSegmentStreetName(segmentId) {
	...
	/**
	 * Get venue street name
	 */
	GetVenueStreetName(venueId) {

		let streetName = null;

		try {

			streetName = WT_WME_SDK.DataModel.Venues.getAddress(
				{ venueId: venueId }
			).street.name;
	...
	/**
	 * Get segment city name
	 */
	GetSegmentCityName(segmentId) {
	...
	/**
	 * Get segment state (or province) name
	 */
	GetSegmentStateName(segmentId) {
	...
	/**
	 * Get segment country name
	 */
	GetSegmentCountryName(segmentId) {
	...
	/**
	 * Get segment country ISO code (2 chars)
	 */
	GetSegmentCountryAbbr(segmentId) {
	...
	/**
	 * Get venue country ISO code (2 chars)
	 */
	GetVenueCountryAbbr(venueId) {

		let countryAbbr = null;

		try {

			countryAbbr = WT_WME_SDK.DataModel.Venues.getAddress(
				{ venueId: venueId }
			).country.abbr;
...
class WT_HtmlObjectsClass {
	...
	constructor() {
		...
		this.idInfoMsgBtnClose = 'WT_InfoMsgBtnCloseID';
	...
	ShowInfo(head, body) {
		...
		const btnClose = this.CreateButton(this.idInfoMsgBtnClose, 'Close', '');
		...
		const INFO_HTML_DIV = (
			...
			'<div style="background-color:White">' +
				'<div style="padding:16px">' +
					'<div style="font-size:125%; font-style:italic; color:Green" align="center">' + head + '</div>' +
		...
		const divInfoBackID = 'WT_divInfoBackID';

		let divInfoBack = document.createElement('div');

		divInfoBack.id = divInfoBackID;
		divInfoBack.style.cssText = this.styleModalBackground;

		unsafeWindow.jQuery('#map').append(divInfoBack);

		// Message

		const divInfoMsgID = 'WT_divInfoMsgID';

		let divInfoMsg = document.createElement('div');

		divInfoMsg.id = divInfoMsgID;
		divInfoMsg.style.cssText = this.styleCenteredDiv;
		divInfoMsg.innerHTML = INFO_HTML_DIV;

		unsafeWindow.jQuery('#map').append(divInfoMsg);

		document.getElementById(this.idInfoMsgBtnClose).addEventListener('click', () => {
			this.DismissDiv([divInfoMsgID, divInfoBackID]);
		});
	}
...
function SDK_DumpVars() {
	...
	try {
		...
		WT_CONSOLE.DebugGroupStart('🔎 Dump of the Waze SDK content');
		...
		WT_CONSOLE.DebugObj('.DataModel.Segments', WT_WME_SDK.DataModel.Segments);
		WT_CONSOLE.DebugObj('.DataModel.Users', WT_WME_SDK.DataModel.Users);
		WT_CONSOLE.DebugObj('.DataModel.Venues', WT_WME_SDK.DataModel.Venues);
		WT_CONSOLE.DebugObj('.DataModel.PermanentHazards.getAllCameras()', WT_WME_SDK.DataModel.PermanentHazards.getAllCameras());
		WT_CONSOLE.DebugObj('.DataModel.PermanentHazards.getCameraById()', WT_WME_SDK.DataModel.PermanentHazards.getCameraById({ cameraId: 383744 }));
		WT_CONSOLE.DebugGroupEnd();
